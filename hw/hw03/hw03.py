...
def missing_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    last = n % 10  # 取最后一位
    all_but_last = n // 10  # 切割前面的数字
    if all_but_last == 0:  # 如果只有一位数字，直接返回0
        return 0
    if last - 1 != all_but_last % 10 and last != all_but_last % 10:
...
def count_coins(change):
    ...
    def func(change, n):
        if change == 0:
            ...
            return func(change, get_next_coin(n)) + func(change - n, n)
...
def make_anonymous_factorial():
    ...
    return lambda n: (lambda n, function: 1 if n == 1 else function(n-1, function) * n)(n, lambda n, function: 1 if n == 1 else function(n-1, function) * n)
